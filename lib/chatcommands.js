  ...
  var commands = {
    ...
    */
    ...
    "shuffleuser": new Command({
      ...
      if (!utils.isValidUserName(TARGETUSER)) {
        bot.sendPM(user.name, "$spreaduser <username> <spacing> -- Disperses a user's videos throughout the playlist.");
        return false;
      }
      let userVideos = bot.getUserVideos(TARGETUSER);
      let pl = bot.CHANNEL.playlist;
      ...
      var spreadVids = function() {
        let pl = bot.CHANNEL.playlist;
        for(let i = 1; i < pl.length; i++){
          if(pl[i].queueby != TARGETUSER) {continue;}
          
          let spacingNeeded = SPACING;
          for(let j = i-1; j >= i - SPACING && j >= 0; j--) {
            if (pl[j].queueby == TARGETUSER) {
              break;
            } else {
              spacingNeeded--;
            }
          ...
          let after = pl[i-1].uid;
          for(let j = i+1; j < pl.length; j++) {
            if (pl[j].queueby != TARGETUSER) {
              bot.moveMedia(pl[j].uid, pl[i-1].uid);
            }
          }
        }
      };
