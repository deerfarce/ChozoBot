  ...
  var commands = {
    "8ball": new Command({
      ...
      var answers = ["It is certain", "It is decidedly so", "Without a doubt",
        ...
        "Outlook not so good", "Very doubtful", "Never", "Of course not", "Of course!"];
    ...
    "bump": new Command({
      ...
      if (bumpPos < 0) return false;

      if (!vidObj) {
        bot.sendPM(user.name, "bump: No videos found to bump."); return false;
      }

      if (bumpPos + 2 >= playlist.length) {
        bot.sendPM(user.name, "bump: The playlist is too short."); return false;
      }

      if (vidObj.media.uid == bot.CHANNEL.currentUID) {
        bot.sendPM(user.name, "bump: Can't bump a video that's currently playing!"); return false;
      }

      if (vidObj.index < BUMP_CAP) {
        bot.sendPM(user.name, "bump: Can't bump a video that's already past the bump position ("+BUMP_CAP+")"); return false;
      }
    ...
    "comment": new Command({
      ...
      let cvd = bot.currentVideoData,
        ...
        bold = bot.cfg.chat.filters.bold,
        ID = currentMedia.id;
      if (ID === cvd.id && (cvd.comments !== null || cvd.commentsDisabled)) {
        ...
        if (cvd.commentsDisabled) {
          ...
          if (authorCode && authorCode.trim() !== "")
            bot.sendChatMsg("["+authorCode+"]" +  bold + "<" + comment.author + ">" +  bold + "[/"+authorCode+"] " + comment.text);
          else
            bot.sendChatMsg(bold + "<" + comment.author + ">" +  bold + " " + comment.text);
          return true;
        }
      } else {
    ...
    "fortune": new Command({
      ...
      return bot.sendChatMsg(fortune[0] + bot.cfg.chat.filters.bold + user.name + "'s fortune: " + fortune[1] + bot.cfg.chat.filters.bold);
    }),
    ...
    */
    ...
    "roll": new Command({
      ...
      if (splitmsg[0] && dicereg.test(splitmsg[0])) {
        ...
        if (rolls > 1 && !offset && sum === max) {
          sndmsg += '!! ' + sndmsg + 'PERFECT ' + sum.toString() + ' !!';
        } else {
          if (op === "+") {
            sum += offset;
          } else if (op === "-") {
        ...
        if (bestRoll.checkem > 1 && combos[bestRoll.checkem - 2] !== undefined) {
          sndmsg = '!! ' + user.name + ' rolled ' + combos[bestRoll.checkem - 2] + bestRoll.num + ' !!';
    ...
    "shuffleuser": new Command({
      ...
      let TARGETUSER = message.split(" ")[0];
      let SPACING = parseInt(message.split(" ")[1]);
      if (!utils.isValidUserName(TARGETUSER) || SPACING < 1) {
        bot.sendPM(user.name, "$spreaduser <username> <spacing> -- Spaces out a user's videos throughout the playlist.");
        return false;
      }
      let userVideos = bot.getUserVideos(TARGETUSER);
      let pl = bot.CHANNEL.playlist;
      if (userVideos.length < 2) {
        bot.sendPM(user.name, "Less than two videos found for " + TARGETUSER);
        return false;
      }
      if (pl.length <= userVideos.length)  {
        bot.sendPM(user.name, "Playlist must have at least two contributors to use this command.");
      ...
      for (let ti = 1; ti < pl.length; ti++) { //targetuser video index
        if (pl[ti].queueby != TARGETUSER) {continue;}

        let spacingNeeded = 0;
        for (let i = ti - 1; i >= ti - SPACING - 1 && i >= 0; i--){
          if (pl[i].queueby == TARGETUSER){
            let currentSpacing = ti - i - 1;
            spacingNeeded = SPACING - currentSpacing;
            break;
          }
        }

        while(spacingNeeded > 0){
          let pi = ti; //padding video index
          for(;pl[pi].queueby == TARGETUSER && pi < pl.length; pi++);
          if (pi == pl.length) {break;}
          bot.moveMedia(pl[pi].uid, pl[ti-1].uid);
          let padVid = pl.splice(pi, 1)[0];
          pl.splice(ti, 0, padVid);
          ti++;
          spacingNeeded--;
        }
      }      
    ...
    "top5useremotes": new Command({
      cmdName: "top5useremotes",
      minRank: bot.RANKS.USER,
      rankMatch: ">=",
      userCooldown: 20000,
      cmdCooldown: 15000,
      ...
      if (bot.db && bot.cfg.db.useTables.users && bot.cfg.db.useTables.emote_data) {
        ...
        if (utils.isValidUserName(spl[0])) {
          if (bot.getSavedUserData(spl[0]).quoteExempt || spl[0].toLowerCase() === bot.username.toLowerCase()) {
            ...
            bot.db.run("getUserMostUsedEmotes", [spl[0]], function(res) {
              if (res && res.rowCount > 0) {
                var txt = res.rows[0].uname + "'s top " + res.rowCount + (res.rowCount === 1 ? " emote: " : " emotes: ");
                for (var i = 0; i < res.rows.length; i++) {
                  if (res.rows[i].emote.length > 40) txt += res.rows[i].emote.substr(0,40) + "...";
                  else txt += res.rows[i].emote;
                  txt += ` (${res.rows[i].sum})`;
                  if (i < res.rows.length - 1) txt += ", ";
                }
                bot.sendChatMsg(txt);
    ...
    "trigger": new Command({
      ...
      let trigger = bot.validateTrigger(message[0]);
      if (trigger !== message[0]) {
        bot.sendPM(user.name, "Invalid trigger character.");
        return false;
      }
      else {
        bot.sendPM(user.name, "Trigger set to: " + trigger);
        bot.trigger = trigger;
      }
      return true;
    }),
