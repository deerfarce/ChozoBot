...
function init(bot) {
  ...
  if (!/^[\w-]{1,30}$/.test(schema)) {
    bot.logger.error("db.js: Room name contains invalid characters! Will not initialize database.");
    return false;
  }
  ...
  if (active && schema.trim() !== "") {
    ...
    */
      ...
      bumpCount: function(values, cb) {
        if (bot.cfg.db.useTables.users && bot.cfg.db.useTables.bump_stats) {
          if (values[1] !== "self" &&
              values[1] !== "others" &&
              values[1] !== "vidya_self" &&
              values[1] !== "vidya_others") return false;
    ...
    function run(query, values, cb) {
      if (!bot.cfg.db.use || isEnded() || !bot.db.active || !/^[\w-]{1,30}$/.test(schema)) return Promise.resolve(false);
